...
from Serializables import ConsultasGenerales
import ABM
...
class Sectordeconsulta(QtWidgets.QDialog):
    ...
    def serializables(self):
        ventanamateriales = ConsultarStockSerializables(self)
        ventanamateriales.exec_()

class ConsultarStockSerializables(QtWidgets.QDialog):
    ser_all = []
    def __init__(self, *args, **kwargs):
        super(ConsultarStockSerializables, self).__init__(*args, **kwargs)
        self.ui = consultarStockSerializables.Ui_Form()
        self.ui.setupUi(self)
        self.ui.se_btn_buscar.clicked.connect(self.buscarArticulo)
        self.ui.se_btn_volver.clicked.connect(self.salir)
        ...
        for cant_ser_bd in range(0, len(self.ser_all)):
            QtWidgets.QTreeWidgetItem(self.ui.se_tabla)
        for posicion in range(0, 3):
            for i in range(0, len(self.ser_all)):
                self.ui.se_tabla.topLevelItem(i).setText(posicion, str(self.ser_all[i][posicion]))

    def buscarArticulo(self):
        try:
            self.valor1 = str(self.ui.se_input_buscar.text())
        except:
            QMessageBox.about(self, "Error!!", "\nIngrese el numero de MAC!!\n")
            self.ui.se_tabla.clear()
            for cant_ser_bd in range(0, len(self.ser_all)):
                QtWidgets.QTreeWidgetItem(self.ui.se_tabla)
            for i in range(0, len(self.ser_all)):
                for posicion in range(0, 3):
        ...
        for i in range(0, len(consulta)):
            self.ui.se_tabla.topLevelItem(0).setText(i, str(consulta[i]))
...
class StockMateriales(QtWidgets.QDialog):
    def __init__(self, *args, **kwargs):
        ...
        for i in range(0, len_resultado):
            ...
            for a in range(0, len(resultado[i])):
                item_0 = QtWidgets.QTreeWidgetItem(self.ui.ma_tabla_datos)
                test = resultado[i][a]
                self.ui.ma_tabla_datos.topLevelItem(i).setText(posicion, _translate("Form", str(test)))
                posicion += 1
    ...
    def consulta(self):
        ...
        for i in resultados:
            if posicion == 3:
                posicion = posicion + 1
            self.ui.ma_tabla_datos.topLevelItem(0).setText(posicion, _translate("Form", str(i)))
            posicion += 1
